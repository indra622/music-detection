...
def beatExtraction(st_features, win_len, PLOT=False):
    ...
    for ii, i in enumerate(toWatch):                                        # for each feature
        ...
        [pos1, _] = peakdet(st_features[i, :], DifThres)           # detect local maxima
...
def mtFeatureExtraction(signal, fs, mt_win, mt_step, st_win, st_step):
    ...
    for i in range(n_feats):        # for each of the short-term features:
        cur_p = 0
        N = len(st_features[i])
        mid_feature_names[i] = f_names[i] + "_" + "std"
        # mid_feature_names[i + n_feats] = f_names[i] + "_" + "mean"
        ...
        while (cur_p < N):
            ...
            mt_features[i].append(numpy.std(cur_st_feats))
            # mt_features[i + n_feats].append(numpy.mean(cur_st_feats))
            #mt_features[i+2*n_feats].append(numpy.std(cur_st_feats) / (numpy.mean(cur_st_feats)+0.00000010))
            cur_p += mt_step_ratio
    return numpy.array(mt_features), st_features, mid_feature_names, f_names
...
def calFeature(filename, win = 30):
    """
